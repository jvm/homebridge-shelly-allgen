...
Forward-looking work only. Add, remove, or correct entries — this file is not a changelog. When something ships, delete its entry and record the change in `CHANGELOG.md`.
...
- Hardware-validate `num()` null-position handling on a Gen2+ cover (uncalibrated `pos: null` path); currently unit-tested only.
